...
u8* Input::read(int size) {
    u8 val[size];
    for(int i = 0; i < size; i++) val[i] = this->file->get();
    return val; //Si se usa, hay que liberarlo des
}
...
Input::Input (string filename) {
    ...
    if (this->file->is_open()) {
        ...
        skip(5);
        if (flags_6 & 0x04)
            this->trainer = read(512);
        this->trainer_size = 512;
        this->prg_rom = read(16384 * this->prg_16_rom);
        this->chr_rom = read(8192 * this->chr_8_rom);
